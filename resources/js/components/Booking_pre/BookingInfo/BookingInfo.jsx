...
export default class BookingInfo extends Component {
    constructor(props) {
        super(props);
        this.state={
            idLP: localStorage.getItem('itemsShoppingCart')?JSON.parse(localStorage.getItem('itemsShoppingCart')).idLP:''
        }
    }
    
    onBooking(){
        // Xử lý thông tin cần -> đủ hết
        // Xử lý combo QT sau:  
        // axios get room -> tìm coi Phòng trống ko => cần id
        // Nếu phòng hết xóa LS -> toast đỏ -> về home
        // axios put room -> giảm slTrong (Trigger) => cần id, sl trống sau khi giảm(rjs, ctl: tùy ý)
        // axios post cus -> thêm KH => cần info KH trên state
        // axios post booking -> thêm Booking => cần idKH lấy từ res của axios KH, idLP, Đến, Đi, Đêm, giá 
        // Xong hết -> toast xanh -> về home
    }
    render() {
        return (
                            ...
                            <Row style={{paddingTop:'10%'}}>
                                <Col style={{fontSize:'1.4vw', fontFamily:'Georgia', textAlign:'center'}}>
                                    ...
                                    <Row style={{ paddingTop:'7%'}} className="button-BookNow">
                                        <Col>
                                            <Link to="/booking">
                                                <button onClick={ ()=>this.onBooking() }><b>BOOK NOW</b></button>
                                            </Link>
                                        </Col>
                                    </Row>
                                </Col>
